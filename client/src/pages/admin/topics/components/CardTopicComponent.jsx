...
const CardTopic = ({ topic }) => {
    ...
    return (
        <div
            ...
            onClick={handleClick}
            title='Haz clic para ver las encuestas de esta temática'
        >
                ...
                <Menu.Items className="absolute right-5 top-5 w-44 origin-top-right divide-y divide-gray-100 rounded-lg bg-white shadow-lg ring-1 ring-black/5 focus:outline-none">
                    <div className="px-1 py-1 ">
                        ...
                        <Menu.Item>
                            {({ active }) => (
                                <button
                                    className={`${active ? 'bg-red-600 text-white' : 'text-red-600'} group flex w-full items-center rounded-md px-2 py-2 text-sm transition-colors duration-300 ease-out`}
                                    onClick={(e) => handleDeleteButtomTopic(e)}
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke={active ? '#ffffff' : '#ff2825'} className="w-6 h-6 mr-2">
