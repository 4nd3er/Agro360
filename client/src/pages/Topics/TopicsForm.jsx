...
const TopicsForm = () => {
    ...
    const searchTopic = (input) => {
        ...
        setSearchForms(() => { return forms.filter(form => form.name.toLocaleLowerCase().includes(input.replace(' ', ''))) })
    }
    ...
    return (
        <section className='min-h-[80vh]'>
            ...
            <hr />
            <main className="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 mr-5 mt-10 mb-8">
                {/* iterate forms  */}
                {searchForms.length ? searchForms.map(form => (
                    <CardForm
                        key={form._id}
                        form={form}
                    />
                )) : searchForms.length <= 0 && searchInput.length > 0 ? <h3 className="text-2xl text-gray-600">Encuesta no encontrada</h3>
